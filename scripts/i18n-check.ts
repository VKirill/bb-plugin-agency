/**
 * Interface translation check: every Russian text passed to tr()/<Tr>, to a
 * self-translating prop of the shared components, to a tab list or to a label
 * helper has an English entry, and no tr() call runs at module top level (it
 * would stay Russian forever).
 *
...
const patterns = [
  ...
  // Labels translated inside a component or helper.
  new RegExp(String.raw`\bsaveLabel\s*=\s*${literal}`, "g"),
  new RegExp(String.raw`\bLIMIT_RULE_GROUP\(\s*${literal}`, "g"),
  // Fallback text of a translated expression: tr(value ?? "…").
  new RegExp(String.raw`\btr\([^"()]*\?\?\s*${literal}`, "g"),
];
// TabBar translates every tab label: tabs={["…", "…"]} or tabs={cond ? [...] : [...]}.
const tabLists = /\btabs=\{([^{}]*)\}/g;
const listItem = new RegExp(literal, "g");
...
for (const file of files(ROOT)) {
  ...
  const found = (raw: string) => {
    ...
    if (!CYRILLIC.test(value) || hasTranslation(value)) return;
    missing.set(value, [...(missing.get(value) ?? []), file.slice(ROOT.length + 1)]);
  };
  for (const pattern of patterns) {
    for (const match of text.matchAll(pattern)) found(match[1]);
  }
  for (const list of text.matchAll(tabLists)) {
    for (const item of list[1].matchAll(listItem)) found(item[1]);
  }
