...
> **Статус:** `0.1.0-alpha.18`, рабочая альфа. Постоянные данные, управляемые
...
## Решения руководителя и явная сдача

`bb agency job state --input-json '{"jobId":"<id>"}'` возвращает полную цель, состояния детей, публикации, историю решений и кандидатов в уроки. Руководитель фиксирует выбранный или изменённый путь через `job decide`: причина, неизвестное, доказательства и следующая проверка. У решений своя ревизия CAS; базовая роль короткая, процедуры доступны в навыке Агентства.

Новые запуски сдают точную опубликованную версию `artifactId/version/hash` через `job submit`. Прогресс и публикация плана не запускают приёмку. Возврат сбрасывает сдачу; родителю нужны завершённые рабочие дети и новый итоговый отчёт. Уже работающие попытки сохраняют прежний протокол до явной сдачи, поэтому обновление не требует перезапуска. Автоматические итоги задач остаются кандидатами до разбора; неприменимые уроки не вставляются целиком в каждый запуск.
...
Карточка в каталоге и английский README — на английском. Экраны, кнопки,
настройки и пустые состояния имеют английский словарь. **Настройки → Язык
Агентства** переключает задачи, комментарии и вопросы владельцу. Новая установка
на первом кадре берёт язык интерфейса BB (`en`, если BB не на русском).

Навыки диспетчера в `skills/agency/` пока на русском. Промпты агентам остаются
английскими, язык отчётов задаётся одной строкой из настройки. Полного
английского SKILL.md в этом релизе нет.

## Модели при меньшей подписке

Стартовые сотрудники предпочитают Claude, затем GPT, затем Grok. Если в этом BB
только Claude, писатель с Grok встаёт на Sonnet того же класса. Если есть Claude
и GPT — используются они, Grok не называется. Запасные модели по лимиту подписки
на новом профиле заполняются из других подключённых семейств того же класса.
Уже собранную команду: Настройки → Машины → Модели сотрудников → Перевести на
доступные модели.

Перед запуском исполнителя или помощника отдельный запрос Jev оценивает **полное задание** (название, бриф, приёмку и контракт), без обрезания и загруженных инструкций. Выбор — из поддерживаемых выбранной моделью уровней, включая **xhigh / max**, если они доступны; результат явно передаётся в BB. Навыки и память подбираются другим запросом. Ошибка, неизвестные возможности модели или уверенность ниже 60% сохраняют настройку профиля; журнал `launch-effort` содержит причину, итоговый уровень и hash/длину плана. Профиль сотрудника и модель не меняются.
...
Агентство ничего не создаёт само. В пустом Агентстве можно взять **четырнадцать
стартовых отделов** (продукт, разработка и её конвейер, дизайн, тексты, реклама,
SEO, маркетинг, соцсети, исследования, офис владельца, инфраструктура,
автоматизация, продажи, администрация) с руководителем, исполнителями,
проверяющим, помощником, регламентом и должностными инструкциями на русском или
английском — или создать свои. На запуск из библиотеки отдела — **не больше двух**
навыков. У текстов, рекламы, дизайна, SEO, маркетинга, соцсетей и исследований
ремесло зафиксировано; у инфраструктуры, автоматизации, продаж и офиса пока кит.
Что каждый стол берёт: [departments.md](skills/agency/references/departments.md).
У отдела-конвейера каждому шагу задан свой CLI: сильная модель планирует, дешёвая
читает код, быстрая пишет, а проверяет модель другого вендора; если CLI не
подключён в BB, сотрудник получает модель роли по умолчанию. Как собрать и
настроить такой отдел —
[skills/agency/references/dev-conveyor.md](skills/agency/references/dev-conveyor.md).
Стартовые записи — обычные данные: их можно менять, отправлять в архив и удалять, а
неизменённые переводятся на другой язык одной кнопкой. Отдел или сотрудник уходит
в архив с сохранённой историей, удалить можно только того, кто ещё не работал.
...
- [Что берёт каждый стартовый отдел](skills/agency/references/departments.md) · [фраза → отдел](skills/agency/references/routing.md)
- [Навык диспетчера](skills/agency/SKILL.md) (0.28.5) · [стол ремесла](skills/agency/references/craft-pack.md)
