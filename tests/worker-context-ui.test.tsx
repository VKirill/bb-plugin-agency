...
async function render(scope:"agent"|"department"="department"){host=document.createElement("div");document.body.append(host);root=createRoot(host);await act(async()=>root!.render(createElement(WorkerContextPanel,{scope,scopeId:scope==="agent"?"agent_aaaaaaaa":"departme_aaaaaaaa",skills:[{label:"example-skill"}],notice:vi.fn()})));}
...
describe("worker context editor",()=>{
 ...
 it("saves the assigned preset with CAS and keeps a rejected draft",async()=>{
  ...
  expect(calls.find(c=>c.method==="saveWorkerContext")?.input).toMatchObject({expectedRevision:2,scope:"department",policy:{skills:{mode:"assigned",names:[]},bbPlugins:{mode:"assigned",names:[]},mcpServers:{mode:"allow",names:[]}}});
...
it("keeps employee profile lists as the only catalogs",async()=>{await render("agent");expect(host!.querySelector('[aria-label="Поиск в контексте"]')).toBeNull();expect(host!.textContent).not.toContain("example-skill");expect(host!.textContent).toContain("Основные списки находятся на вкладках");});
