...
- [ ] песочница Claude Code на машине пропускает `bb` к локальному адресу BB-хоста. На OVH (AG-8) команды `bb agency` изнутри песочницы не достали до `127.0.0.1`, первая публикация упала по тайм-ауту, и агент выполнил их вне песочницы. Разрешение настраивает владелец машины (`/sandbox` в Claude Code); без него работа проходит только ценой выхода из песочницы.
