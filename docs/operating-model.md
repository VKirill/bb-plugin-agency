...
- [x] Правило «Запуск без песочницы» (Агентство → отдел → сотрудник), по умолчанию выключено; запуск передаёт `permissionMode: full` с явным источником (проверено тестами, живой запуск с правилом не делался).
