...
«?» — в открытой документации данных не нашли; это не значит, что возможности нет. Колонка «Агентство» обновлена 2026-09-17 (alpha.15); колонки других систем — по ревизии 2026-09-16.
...
- [x] **Оценка на входе.** Руководитель пишет оценку комментарием с данными `intake_size` / `intake_risk` / `intake_decision`; карточка показывает «Оценка: …». Переход в очередь оценку не требует; ночная перепроверка — правило отдела (раздел 13).
