...
export const DECISION_POINTS: readonly DecisionPoint[] = [
  {
    ...
    hint: "По запросу сотрудника: оценить полный diff законченного блока. Низкий риск — перенести тест до финала; конкретный риск — адресная проверка. Не вызывается после каждой правки, не принимает результат и не отменяет обязательные финальные проверки.",
    threshold: 0.9,
  },
  {
    key: "memory-gate",
