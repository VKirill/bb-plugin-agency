...
export function buildWorkerInstructions(worker: WorkerContext): string {
  if (worker.isLead) {
    const head = [
      ...
      "Before assigning implementation, identify what is known and unknown. Obtain available facts yourself or through focused discovery/spikes; for software adaptation compare the product with the installed SDK/API and runtime. Missing discoverable facts are not an owner decision. Maintain one plan covering the whole goal; revise it for changed scope, interfaces or delivery route, not every local fix; the owner need not prescribe these steps.",
    ];
    ...
    const tail = [
      ...
      "3. When a child changes state, the Agency messages this thread. Read the changed child and resolve actionable blockers before waiting; scan the whole graph only when dependencies or the route change. Give a bounded next milestone and an estimate, revising it when facts change. Use job decide for a changed route: unknowns, bottleneck, action, rationale, evidence and nextCheck; expectedRevision is decisionRevision. Do not write decisions for every tool call.",
      ...
      "Once implementation has no named blocking defect and its required candidate checks pass, hand the exact version to the existing reviewer. Do not prolong open-ended audits; name any missing criterion before more checking. Record decisions at route changes and report milestones or blockers, not every command.",
  ...
  const common = [
    ...
    "- If the job has an execution contract, start with readFirst, keep interfaces as they are, change only what mayChange lists, leave mustNotTouch alone, run its required checks at the required milestone; put commands and results once in the report and link it in the final comment. Going outside it is a question to the lead, not a decision.",
    "- Implement a coherent batch before verification. Text-only edits do not require a full code suite. Run an intermediate focused check for a concrete failure, risky boundary or required checkpoint; the reviewer normally runs full mandatory checks on the completed candidate. Reuse a valid same-version author run while independently checking critical behavior; do not make author and reviewer duplicate the whole suite. Repeat only invalidated checks. Index after substantial source changes, not each edit. Optional for an uncertain completed batch: bb agency call assessVerification with full diff and changedPaths; its advice never replaces final acceptance. Do not call it per edit.",
