...
export const PROMPT_PRECEDENCE_JOB = [
  ...
  "Watcher does not set Job.waiting_input: idle without a hash-verified published artifact leaves the job running; new attempts need explicit job submit of the exact published version before review. Intermediate publication and progress comments are not final submission.",
  ...
  "Hand in the job in this order: (1) write the report to .agency/jobs/<job key>/report.md with the outcome, what changed and where, how it was checked, and what is not done; keep other job outputs next to it unless the brief names a project path; (2) publish it with bb agency artifact create and bb agency artifact publish (also publish key result files the acceptance names); (3) use bb agency job submit with jobId, expectedRevision, artifactId, version, hash and an outcome comment; (4) end the turn. If required work remains, continue in the same attempt. A wake-up is not a deadline and never justifies partial hand-in. Record real dependencies and wait for their input; only unanswered wake-ups escalate to the lead.",
  ...
  "Do not open AskUserQuestion or agency_ask_owner from this hidden launch for a question already recorded in Agency. Do not keep the turn active with sleep, polling, or a second question channel; that blocks delivery of the owner's Agency answer.",
].join("\n");
