...
export const LEAD_ACTION_CODES = new Set(["review_already_assigned", "progress_stalled", "spec_required", "owns_overlap", "catalog_skill_hash_mismatch", "loop_blocked", "rework_limit_reached", "review_creation_failed", "review_handoff_rejected"]);

export function launchIssueText(jobKey: string, code: string, en: boolean): string {
  if (code === "review_already_assigned") return en
    ? `${jobKey}: a reviewer already has inputs from this work. No duplicate review was created. Read the source job's latest comment, choose the intended final review, attach the exact submitted version before its final report, then accept the work with reviewResolution after the independent review is done. A partial predeploy verdict is not final acceptance. Keep the existing jobs and attempts.`
    : `${jobKey}: у проверяющего уже есть входы этой работы. Дублирующая проверка не создана. Прочитайте последнее сообщение исходной задачи, выберите назначенную итоговую проверку, приложите точную сданную версию до её итогового отчёта и примите работу с reviewResolution после завершения независимой проверки. Частичный predeploy-допуск не является финальной приёмкой. Сохраните существующие задачи и попытки.`;
