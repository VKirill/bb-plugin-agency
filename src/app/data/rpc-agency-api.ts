...
export function createRpcAgencyApi(rpc: RpcCaller): AgencyApi {
  return {
    ...
    listIdeas: (input) => mutate(rpc, STAGE1_RPC.listIdeas, input),
    getIdea: (input) => mutate(rpc, STAGE1_RPC.getIdea, input),
    saveIdea: (input) => mutate(rpc, STAGE1_RPC.saveIdea, input),
    setIdeaStatus: (input) => mutate(rpc, STAGE1_RPC.setIdeaStatus, input),
    spawnIdeaThread: (input) => mutate(rpc, STAGE1_RPC.spawnIdeaThread, input),
    ...
    starterKit: (input) => mutate(rpc, STAGE1_RPC.starterKit, input),
    installStarterKit: (input) => mutate(rpc, STAGE1_RPC.installStarterKit, input),
    translateStarterKit: (input) => mutate(rpc, STAGE1_RPC.translateStarterKit, input),
    recordLifecycle: (input) => mutate(rpc, STAGE1_RPC.recordLifecycle, input),
    archiveDepartment: (input) => mutate(rpc, STAGE1_RPC.archiveDepartment, input),
    restoreDepartment: (input) => mutate(rpc, STAGE1_RPC.restoreDepartment, input),
    deleteDepartment: (input) => mutate(rpc, STAGE1_RPC.deleteDepartment, input),
    deleteAgent: (input) => mutate(rpc, STAGE1_RPC.deleteAgent, input),
