...
import { dueAtFromDate, parseDescription, PRIORITY_CODE } from "./view-models";
...
export function failureNotice(failure: MutationFailure): string {
  ...
  if (failure.kind === "domain") {
    return DOMAIN_NOTICE[failure.error.code] ?? productDomainNotice(failure.error.code, failure.error.message);
  }
...
function draftRevisionConflict(expected: number | undefined, actual: number): MutationFailure | null {
  if (expected === undefined) {
    return { kind: "domain", error: { code: "invalid_command", message: "Нет ревизии черновика." } };
  }
  if (expected !== actual) {
    return {
...
export async function persistJobPatch(
  ...
  if (!record) return { ok: false, failure: { kind: "domain", error: { code: "not_found", message: "Задача не найдена на сервере." } } };
  ...
  if ((next.bindingId || next.department || next.project) && (!bindingId || !departmentId)) {
    return { ok: false, failure: { kind: "domain", error: { code: "placement_required", message: "Укажите проект и отдел из каталога. Первый в списке не подставляется." } } };
  }
  if (bindingId && departmentId && !snapshot.projectDepartments.some((row) => row.bindingId === bindingId && row.departmentId === departmentId)) {
    return { ok: false, failure: { kind: "domain", error: { code: "department_not_on_binding", message: "Отдел не привязан к этому проекту." } } };
  }
...
export async function persistAgentPatch(
  ...
  if (!record) return { ok: false, failure: { kind: "domain", error: { code: "not_found", message: "Сотрудник не найден на сервере." } } };
  ...
  const policyVersionId = currentVersion?.policyVersionId || snapshot.policies[0]?.id;
  if (!policyVersionId) {
    return { ok: false, failure: { kind: "domain", error: { code: "not_found", message: "Нет политики для версии профиля." } } };
  }
  ...
  const providerId = next.selection.providerId.trim() || currentVersion?.providerId || "codex";
  const model = next.selection.model.trim() || currentVersion?.model || "default";
...
export async function persistDepartmentPatch(
  ...
  if (!record) return { ok: false, failure: { kind: "domain", error: { code: "not_found", message: "Отдел не найден на сервере." } } };
  const stale = draftRevisionConflict(next.revision, record.revision);
  if (stale) return { ok: false, failure: stale };
  const lead = requireExactId(snapshot.agents, next.lead, "Руководитель отдела должен быть выбран из каталога по идентификатору.");
  if (!lead.ok) return lead;
  const wantedIds: string[] = [];
  for (const token of next.members) {
    const member = requireExactId(snapshot.agents, token, "В составе отдела есть неизвестный сотрудник. Имя не подставляется.");
  ...
  const processChanged =
    ...
    (next.instructions !== currentProcess?.instructions || next.acceptance !== currentProcess?.acceptance);
    ...
    ...(processChanged
      ? {
          process: {
            ...
            reviewPolicy: currentProcess?.reviewPolicy ?? { required: true },
          },
        }
      : {}),
    memberships: wantedIds.map((agentId) => ({ agentId, role: agentId === lead.id ? "lead" : "member" })),
  });
  return result.ok ? { ok: true } : result;
}
...
export async function persistProjectPatch(
  ...
  if (!record) return { ok: false, failure: { kind: "domain", error: { code: "not_found", message: "Привязка проекта не найдена на сервере." } } };
  const currentLinked = snapshot.projectDepartments
    .filter((row) => row.bindingId === record.id)
    .map((row) => row.departmentId);
  const wanted: string[] = [];
  for (const token of next.members) {
    const linked = requireExactId(snapshot.departments, token, "К проекту подключён неизвестный отдел. Название не подставляется.");
...
export function canCreateJob(
  ...
  if (!snapshot.bindings.some((binding) => binding.id === bindingId)) return null;
  if (!snapshot.departments.some((department) => department.id === departmentId)) return null;
  if (!snapshot.projectDepartments.some((row) => row.bindingId === bindingId && row.departmentId === departmentId)) {
    return null;
  }
...
export async function openPersistedArtifact(
  ...
  if (actual !== opened.value.hash || (input.expectedHash && input.expectedHash !== opened.value.hash)) {
    return { ok: false, failure: { kind: "domain", error: { code: "artifact_hash_mismatch", message: "Хеш открытого файла не совпал с записью версии." } } };
  }
  return opened;
}
