...
export function FilePicker({onFiles,notice,label="Прикрепить файл"}:{onFiles:(files:TaskFile[])=>void;notice:(s:string)=>void;label?:string}) {
 ...
 return <><input ref={input} type="file" multiple accept=".md,.txt,.json,.csv,image/png,image/jpeg,image/webp,image/gif" className="hidden" aria-label={label} onChange={async e=>{
 ...
 const isImage=/^image\/(png|jpeg|webp|gif)$/.test(file.type);const isText=/\.(md|txt|json|csv)$/i.test(file.name);
...
export function FilePreview({file,close}:{file:TaskFile|null;close:()=>void}) {
 return <Dialog open={Boolean(file)} onOpenChange={v=>{if(!v)close();}}><DialogContent className="sm:max-w-2xl"><DialogHeader><DialogTitle>{file?.name}</DialogTitle><DialogDescription>Предпросмотр вложения в прототипе. Файл не загружен на сервер.</DialogDescription></DialogHeader><div className="max-h-[65dvh] overflow-auto">{file?.kind==="image"?<img src={file.content} alt={file.name} className="max-h-[60dvh] max-w-full object-contain"/>:file&&(/\.md$/i.test(file.name)?<Markdown content={file.content}/>:<pre className="whitespace-pre-wrap break-words text-sm">{file.content}</pre>)}</div></DialogContent></Dialog>;
}
