      ...
      <HintHeading
        ...
        hint={
          <>
            ...
            <p>{tr("Оценщик читает задание и открывает отсюда то, без чего работа будет заметно хуже — чтобы результат был качественнее: на один запуск и с записью в журнал.")}</p>
            <p>{tr("Правит библиотеку владелец или руководитель этого отдела. Пусто — работают только профили сотрудников.")}</p>
            <p>{tr("Сохранение в карточке заменяет список целиком. Чтобы дописать навык, не затирая остальные, через CLI: pool-save с mode merge.")}</p>
          </>
        }
      />
