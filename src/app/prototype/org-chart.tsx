...
export function OrgChart({group,kind,agents,departments,openAgent,openDepartment}:Props) {
 ...
 function person(ref:string,label:string,primary=false) {
  ...
  return <button type="button" disabled={!agent} onClick={()=>agent&&openAgent(agent.id)} className={`w-56 rounded-lg border border-border p-3 text-left transition-colors hover:bg-muted focus-visible:outline focus-visible:outline-ring ${primary?"bg-muted/60":"bg-background"}`} aria-label={tr("Открыть сотрудника {name}", { name: shown })}>
   <span className="mb-2 block text-xs text-muted-foreground">{tr(label)}</span>
   <span className="flex items-center gap-2.5">{agent?<AgentMark id={agent.selection.providerId||""}/>:<Icon name="User" className="size-5"/>}<span className="min-w-0"><span className="block break-words text-sm font-semibold">{shown}</span><span className="mt-0.5 block text-xs text-muted-foreground">{agent?.role||tr("Сотрудник недоступен")}{agent&&!agent.enabled?tr(" · На паузе"):""}</span></span></span>
  </button>;
 }
 return <section aria-label={tr("Организационная схема")} className="space-y-3">
  <div className="flex flex-wrap gap-x-5 gap-y-2 text-xs text-muted-foreground"><span className="flex items-center gap-2"><span aria-hidden className="w-6 border-t border-foreground/40"/>{tr("Подчинение в отделе")}</span>{isProject&&<span className="flex items-center gap-2"><span aria-hidden className="w-6 border-t border-dashed border-foreground/40"/>{tr("Координация в проекте")}</span>}</div>
  <div className="overflow-x-auto rounded-xl border border-border bg-muted/20 p-5" tabIndex={0} aria-label={tr("Схема команды, доступна горизонтальная прокрутка")}>
   <div className="flex min-w-max flex-col items-center py-3">
    ...
    {members.length>0&&<><div aria-hidden className={`h-8 border-l border-foreground/30 ${isProject?"border-dashed":""}`}/><ul aria-label={tr(isProject?"Подключённые отделы":"Подчинённые сотрудники")} className="flex list-none p-0">
     {members.map((name,i)=>{
    ...
    {!members.length&&<p className="mt-6 max-w-xs text-center text-sm text-muted-foreground">{tr(isProject?"Отделы ещё не подключены.":"Кроме руководителя, сотрудников пока нет.")} {tr("Добавьте их в режиме «Состав».")}</p>}
   </div>
  </div>
  <p className="text-xs text-muted-foreground">{tr("Нажмите на имя, чтобы открыть карточку.")}{isProject?tr(" Команды отделов можно свернуть. Один сотрудник может участвовать в нескольких отделах."):""}</p>
 </section>;
}
