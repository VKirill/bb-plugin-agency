...
const TEMPLATE_META: Record<TemplateKey, { title: string; hint: ReactNode }> = {
  ...
  playbookLead: {
    title: "Базовая инструкция руководителя",
    hint: <><p><Tr text={"Общий порядок работы: как принять поручение, оценить, разбить, назначить, проверить и собрать итог."}/></p><p><Tr text={"Доходит до каждого запуска руководителя отдельным слоем — копировать его в должностные инструкции не нужно."}/></p></>,
  },
  playbookExecutor: {
    title: "Базовая инструкция исполнителя",
    hint: <><p><Tr text={"Общий порядок: сверить с собой, проверить входы, найти навык, сделать, проверить себя, сдать версией."}/></p><p><Tr text={"Доходит до каждого запуска исполнителя отдельным слоем."}/></p></>,
  },
  playbookReviewer: {
    title: "Базовая инструкция проверяющего",
    hint: <><p><Tr text={"Общий порядок проверки: открыть версию, пройти по критериям, воспроизвести, описать дефекты, дать вердикт."}/></p></>,
  },
  playbookAssistant: {
    title: "Базовая инструкция помощника",
    hint: <><p><Tr text={"Общий порядок: взять только названное, не додумывать, дать ссылку на каждый факт, сдать выжимку."}/></p></>,
  },
  jobDescriptionAssistant: {
    title: "Должностная инструкция помощника",
    hint: <><p><Tr text={"Подставляется, когда сотрудника добавляют в отдел помощником."}/></p><p><Tr text={"Помощник готовит материал для сотрудника: читает, ищет, собирает. Решения принимает тот, кому он помогает."}/></p></>,
  },
  brief: {
    title: "Бриф задачи",
