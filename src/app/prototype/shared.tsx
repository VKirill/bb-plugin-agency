...
export function Rows({ rows }: { rows: [string, ReactNode][] }) { return <dl className="divide-y divide-border">{rows.map(([k,v]) => <div key={k} className="flex flex-wrap justify-between gap-3 py-2 text-sm"><dt className="text-muted-foreground">{tr(k)}</dt><dd className="max-w-full break-words">{trNode(v)}</dd></div>)}</dl>; }
