...
export const AGENCY_RULE_GROUPS: RuleGroup[] = [
  {
    ...
    fields: [
      ...
      { key: "nightlyRecheck", label: "Перепроверять принятое за день", kind: "bool", hint: <><p><Tr text={"Включено — раз в сутки Агентство собирает версии отдела, принятые после прошлой перепроверки, и ставит проверяющему задачу перепроверить их: по одной на папку проекта, до 20 версий за ночь."}/></p><p><Tr text={"Приёмка остаётся в силе. Найденные дефекты попадают в отчёт перепроверки, решение за владельцем. Без проверяющих в отделе задачу получает руководитель."}/></p></> },
      { key: "nightlyRecheckHour", label: "Час перепроверки", unit: "ч", kind: "int", min: 0, max: 23, hint: <p><Tr text={"Час по часам машины BB, с которого запускается перепроверка этих суток. 3 — в три часа ночи."}/></p> },
  ...
  {
    title: "Память отдела",
    hint: <p><Tr text={"После приёмки главной задачи Агентство собирает урок из её фактов: сколько было кругов доработки, из-за чего возвращали, сколько заняло. Урок приходит строкой в каждый запуск отдела, полный текст сотрудник берёт сам."}/></p>,
    fields: [
      { key: "autoLearn", label: "Отдел учится сам", kind: "bool", hint: <><p><Tr text={"Включено — кандидаты в уроки доступны руководителю в состоянии задачи. Он проверяет причину, исправление и результат, затем принимает полезный урок без вашего участия."}/></p><p><Tr text={"Выключено — кандидаты остаются предложениями в «Знаниях» для ручного разбора. Статистика задачи сама по себе не становится правилом."}/></p><p><Tr text={"Принять или убрать запись отдела может и его руководитель; знания проекта и всего Агентства остаются за владельцем."}/></p></> },
      { key: "memoryLimit", label: "Записей в памяти отдела", kind: "int", min: 5, max: 200, hint: <><p><Tr text={"Сколько принятых записей отдел держит в работе. Сверх этого числа самые старые и наименее важные уходят в архив — индекс в запуске не растёт бесконечно."}/></p><p><Tr text={"Закреплённая запись не вытесняется никогда."}/></p></> },
      { key: "memoryTtlDays", label: "Срок жизни урока", unit: "дн", kind: "int", min: 7, max: 365, hint: <><p><Tr text={"Через этот срок урок, который написало Агентство, уходит в архив: память не копит правила, которые никто не подтвердил."}/></p><p><Tr text={"Закреплённый урок и запись, написанная вами, живут без срока."}/></p></> },
    ],
  },
  {
...
/** The rule a machine sets for every launch on it. */
export const HOST_SANDBOX_RULE_GROUP: RuleGroup = {
  title: "Песочница на этой машине",
  hint: (
    <>
      <p><Tr text={"Правило для всех запусков на машине. «Как в правилах отдела» — решает отдел; «Да» — без песочницы, например на Linux-сервере, где песочница не пускает команды Агентства; «Нет» — всегда в песочнице."} /></p>
      <p><Tr text={"Личное правило сотрудника сильнее правила машины."} /></p>
    </>
  ),
  fields: [SANDBOX_FIELD],
};
