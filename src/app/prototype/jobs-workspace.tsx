...
export function JobsWorkspace({jobs,setJobs,agents,projects,departments,scope,setScope,open,go,persist,notice}:{jobs:Job[];setJobs:(jobs:Job[])=>void;agents:Agent[];projects:Group[];departments:Group[];scope:TaskScope;setScope:(scope:TaskScope)=>void;open:(id:string)=>void;go:(section:string)=>void;persist?:(job:Job)=>void|Promise<boolean>;notice?:(text:string)=>void}) {
 ...
 return <div className="agency-jobs-workspace"><div className="agency-jobs-grid">
  <div className="agency-jobs-main min-w-0"><JobsPage jobs={jobs} setJobs={setJobs} open={open} scope={scope} persist={persist} notice={notice} requirePlacement placements={{projects,departments,projectDepartments:linksFromProjects(projects,departments)}}/></div>
  <aside aria-label="Выбор задач" className="agency-jobs-nav min-w-0 rounded-xl border border-border bg-muted/20 p-3" data-expanded={expanded}>
   ...
   <div id="agency-task-scopes" className="agency-jobs-nav-content space-y-5">
    ...
    <section><div className="mb-2 flex items-center justify-between px-2"><h2 className="text-xs font-semibold text-muted-foreground">Проекты</h2><button aria-label="Управлять проектами" onClick={()=>go("projects")} className="rounded p-1 text-muted-foreground hover:bg-muted focus-visible:outline focus-visible:outline-ring"><Icon name="Settings" className="size-3.5"/></button></div>{projects.map(p=>item({kind:"project",value:p.id},p.name,<Icon name="Folder" className="size-4"/>))}</section>
