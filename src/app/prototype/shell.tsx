...
export function AgencyPrototype({ subPath }: PluginNavPanelProps) {
 ...
 return <div className="flex h-full min-h-0 min-w-0 flex-col bg-background text-foreground"><div className="flex flex-wrap items-center justify-between gap-2 border-b border-border bg-muted/40 px-4 py-2 text-xs"><span><strong>Прототип</strong> · Пример данных, изменения до перезагрузки. Агенты не запускаются.</span><Button variant="ghost" size="sm" onClick={()=>setResetOpen(true)}>Сбросить пример</Button></div>
 <div className="flex min-h-0 min-w-0 flex-1"><nav aria-label="Разделы агентства" className="hidden w-44 shrink-0 space-y-1 overflow-y-auto border-r border-border px-2 py-4 lg:block">{sections.map(([key,label,icon])=><button key={key} aria-current={section===key?"page":undefined} onClick={()=>go(key)} className={`flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-sm transition-colors hover:bg-muted ${section===key?"bg-muted font-medium":"text-muted-foreground"}`}><Icon name={icon} className="size-4"/>{label}</button>)}</nav>
 <div className="min-w-0 flex-1 overflow-y-auto"><div className="border-b border-border p-3 lg:hidden"><Choice label="Раздел агентства" value={section} onChange={s=>go(s)} options={sections.map(([value,label])=>({value,label}))}/></div><div className="mx-auto w-full max-w-7xl space-y-4 p-4 md:p-6">{message&&<div role="status" className="flex items-center justify-between gap-3 rounded-md border border-border bg-muted/50 px-3 py-2 text-sm"><span>{message}</span><Button size="sm" variant="ghost" aria-label="Закрыть сообщение" onClick={()=>setMessage("")}>×</Button></div>}{page}</div></div></div>
