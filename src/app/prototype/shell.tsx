...
import { IdeasLivePage } from "./ideas-live";
...
function AgencyShell({ subPath }: PluginNavPanelProps) {
 ...
 const allDepartments=demoMode?demoDepartmentsState:workspace.departments;
 // Archived departments stay in their own list only: routing, forms and pickers never offer them.
 const departments=useMemo(()=>allDepartments.filter(item=>!item.archivedAt),[allDepartments]);
 ...
 const group=(section==="projects"?projects:allDepartments).find(g=>g.id===id);
 ...
 else if(section==="projects"||section==="departments")page=id?(group?<GroupDetail key={`${section}-${id}`} kind={section} group={group} catalogSkills={workspace.catalog?.skills??[]} update={g=>{if(demoMode){if(section==="projects")setDemoProjectsState(demoProjectsState.map(x=>x.id===g.id?g:x));else setDemoDepartmentsState(demoDepartmentsState.map(x=>x.id===g.id?g:x));return;}if(section==="projects")workspace.setProjects(projects.map(x=>x.id===g.id?g:x));else workspace.updateDepartment(g);}} jobs={jobs} setJobs={demoMode?setDemoJobsState:workspace.setJobs} persist={persistJob} openJob={id=>go("jobs",id)} back={()=>go(section)} notice={workspace.setMessage} agents={agents} departments={departments} openAgent={id=>go("agents",id)} openDepartment={id=>go("departments",id)} people={section==="projects"?departments.map(d=>d.name):agents.map(a=>a.name)} projects={projects} live={!demoMode} commit={demoMode?undefined:section==="departments"?workspace.commitDepartment:workspace.commitProject} projectActions={demoMode?undefined:workspace.projectActions}/>:(!demoMode&&workspace.status==="loading"?<Empty title="Загружаем" description="Читаем запись из постоянного хранилища."/>:<Empty title="Запись не найдена" description="Вернитесь к списку."/>)):<GroupsPage groups={section==="projects"?projects:allDepartments} kind={section} open={id=>go(section,id)} create={()=>newGroup(section)} agents={agents} live={!demoMode} notice={workspace.setMessage}/>;
 ...
 else if(section==="ideas")page=demoMode?<Empty title="Идеи" description="Идеи работают на данных Агентства: выключите пример."/>:<IdeasLivePage projects={projects.filter(item=>!item.archivedAt).map(item=>({id:item.recordId??item.id,name:item.bbProjectName||item.name,hostId:item.hostId,root:item.root}))} selectedId={id} openIdea={ideaId=>ideaId?go("ideas",ideaId):go("ideas")} notice={workspace.setMessage}/>;
 else if(section==="inbox")page=<InboxPage jobs={jobs} agents={agents} readIds={readIds} setReadIds={setReadIds} go={go} dispatcher={demoMode?undefined:dispatcherApi} messages={demoMode?undefined:dispatcherApi} notice={workspace.setMessage}/>;
