...
import { KnowledgePage, seedMaterials } from "./knowledge";
...
export function AgencyPrototype({ subPath }: PluginNavPanelProps) {
 ...
 const [materials,setMaterials]=useState(()=>structuredClone(seedMaterials));
 ...
 const reset=()=>{setSettings({...seedSettings});setMaterials(structuredClone(seedMaterials));setReadIds([]);setJobs(structuredClone(seedJobs));setAgents(structuredClone(seedAgents));setDepartments(structuredClone(seedDepartments));setProjects(structuredClone(seedProjects));setAutomations(structuredClone(seedAutomations));setResetOpen(false);setTaskScope({kind:"all"});go("jobs");setMessage("Пример сброшен.");};
 ...
 else if(section==="inbox")page=<InboxPage jobs={jobs} agents={agents} readIds={readIds} setReadIds={setReadIds} go={go}/>;
 else if(section==="runs")page=id&&id!=="RUN-204"?<Empty title="Запуск не найден" description="Выберите запуск из списка."/>:<RunsPage openJob={()=>go("jobs","AG-102")} detail={Boolean(id)} open={()=>go("runs","RUN-204")} back={()=>go("runs")} notice={setMessage}/>;
 else if(section==="knowledge")page=<KnowledgePage items={materials} setItems={setMaterials} scopes={[...projects.map(p=>p.name),...departments.map(d=>d.name)]}/>;
 else if(section==="settings")page=<SettingsPage settings={settings} setSettings={setSettings} hosts={hosts} notice={setMessage} reset={()=>setResetOpen(true)}/>;
 else page=<Empty title="Страница не найдена" description="Выберите раздел в навигации."/>;
 return <div className="flex h-full min-h-0 min-w-0 flex-col bg-background text-foreground"><div className="flex flex-wrap items-center justify-between gap-2 border-b border-border bg-muted/40 px-3 py-1 text-xs"><span><strong>Прототип</strong> · Задачи и сотрудники — пример. Настройки машин и подключений сохраняются. Агенты не запускаются.</span><Button variant="ghost" size="sm" onClick={()=>setResetOpen(true)}>Сбросить пример</Button></div>
 <div className="flex min-h-0 min-w-0 flex-1"><nav aria-label="Разделы агентства" className="hidden w-40 shrink-0 space-y-0.5 overflow-y-auto border-r border-border px-2 py-2 lg:block">{[{name:"Работа",keys:["jobs","inbox"]},{name:"Команда",keys:["projects","departments","agents"]},{name:"Управление",keys:["automations","knowledge","runs","settings"]}].map(group=><div key={group.name} className="pb-4"><p className="px-2 pb-2 pt-2 text-xs font-medium text-muted-foreground">{group.name}</p>{group.keys.map(key=>{const item=sections.find(s=>s[0]===key)!;return <button key={key} aria-current={section===key?"page":undefined} onClick={()=>go(key)} className={`flex w-full items-center gap-2 rounded-md px-2 py-2 text-left text-xs transition-colors hover:bg-muted ${section===key?"bg-muted font-medium":"text-muted-foreground"}`}><Icon name={item[2]} className="size-4"/>{item[1]}</button>;})}</div>)}</nav>
