...
function AgencyShell({ subPath }: PluginNavPanelProps) {
 ...
 else if(section==="agents")page=id?(agent?<AgentDetail key={id} primaryHostId={primaryHostId} agent={agent} update={a=>demoMode?setDemoAgentsState(demoAgentsState.map(x=>x.id===a.id?a:x)):workspace.updateAgent(a)} hosts={hosts} back={()=>go("agents")} notice={workspace.setMessage} live={!demoMode} catalog={workspace.catalog} commit={demoMode?undefined:workspace.commitAgent} policies={workspace.snapshot.policies.map(policy=>({id:policy.id,summary:policySummary(policy,id=>hosts.find(host=>host.id===id)?.name??id)}))}/>:(!demoMode&&workspace.status==="loading"?<Empty title="Загружаем сотрудника" description="Читаем профиль из постоянного хранилища."/>:<Empty title="Сотрудник не найден" description="Выберите профиль из списка."/>)):<AgentsPage agents={agents} open={id=>go("agents",id)} create={newAgent}/>;
