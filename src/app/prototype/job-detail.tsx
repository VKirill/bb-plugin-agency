...
const nextState: Partial<Record<State, string>> = {
 backlog:"Поручение без исполнителя остаётся в бэклоге. Когда назначен сотрудник, Агентство само ставит его в очередь запуска.",queued:"Поручение в очереди запуска: сотрудник стартует сам, когда среда и лимиты позволят. Кнопки нужны только если очередь сняла задачу.",
 running:"Исполнитель готовит результат.",review:"Результат ожидает проверки.",waiting_input:"Вопрос ушёл в чат, откуда ставили задачу. Карточка — запасной вход.",blocked:"Работа остановлена. Причина — в истории задачи: верните задачу в очередь и запустите снова или отмените.",done:"Результат принят.",canceled:"Задача отменена.",
};
export function JobDetail({agents,projects=[],departments=[],job,jobs,update,addJob,openJob,openAgent,openDepartment,openProject,back,notice,openRun,openUsage,runs=[],demoMode=false}:Props) {
 ...
 const refreshLive=useCallback(async()=>{
  ...
  setRemoteActivity(mapActivity(envelope.value.activity??[],agents,{jobId:envelope.value.job?.id,keyOf:(id)=>jobsRef.current.find(item=>item.recordId===id)?.id}));
 ...
 return <article data-document-visible={documentVisible} className="agency-task-layout mx-auto w-full max-w-[1120px] px-1 pb-8 sm:px-4">
  ...
  <div className="agency-task-grid">
   ...
   <section aria-label={tr("Текущее состояние")} className={`rounded-xl border p-4 ${job.state==="review"?"border-foreground/20 bg-muted/60":"border-border bg-muted/30"}`}>
    <div className="flex items-start gap-3"><span className="mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-full border border-border bg-background">{job.state==="review"?<Icon name="Eye" className="size-4 text-blue-500"/>:job.state==="running"?<Icon name="Play" className="size-4 text-amber-500"/>:job.state==="done"?<Icon name="CircleCheck" className="size-4 text-emerald-500"/>:job.state==="blocked"||job.state==="waiting_input"?<Icon name="AlertCircle" className="size-4 text-orange-500"/>:<Icon name="Circle" className="size-4 text-muted-foreground"/>}</span><div className="min-w-0 flex-1"><span className="text-[11px] font-medium uppercase tracking-wider text-muted-foreground">{job.state==="review"?tr("Конвейер"):tr("Текущий этап")}</span><h2 className="mt-0.5 text-sm font-semibold">{job.state==="review"?tr("На проверке: линия закроет станцию сама"):job.state==="done"?tr("Результат принят"):tr(nextState[job.state]??"")}</h2>   <p className="mt-1 text-xs text-muted-foreground">{tr("Исполнитель: {agent}",{agent:job.agent})}</p>
    {stageFile&&<div className="mt-4 flex flex-wrap gap-2"><FileChip file={stageFile} open={selectFile}/></div>}
    {job.state==="review"&&(demoMode?<JobAcceptControls job={job} files={files} api={api} demoMode notice={notice} onAccepted={(text)=>change({state:"done"},text)} secondary={<Button size="sm" variant="outline" onClick={()=>setReviewing(true)}>{tr("Вернуть с замечанием")}</Button>}/>:<div className="mt-4"><Button size="sm" variant="outline" onClick={()=>setReviewing(true)}>{tr("Вернуть с замечанием")}</Button></div>)}{job.state==="done"&&!job.parentId&&!demoMode&&<div className="mt-4 flex flex-wrap items-center gap-2" data-testid="job-reclaim"><Button size="sm" variant="outline" onClick={()=>setReviewing(true)}>{tr("Вернуть продукт с замечанием")}</Button><p className="w-full text-xs text-muted-foreground">{tr("Линия сама проверила и закрыла задачу. Если продукт не тот, верните его целиком: руководитель решит, какую станцию переделать.")}</p></div>}{job.state==="backlog"&&<div className="mt-4 flex flex-wrap items-center gap-2"><Button size="sm" onClick={()=>setStatus("queued")}>{tr("Поставить в очередь")}</Button></div>}
