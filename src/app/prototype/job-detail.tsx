...
import { activityRoleLabel, jobLifecycleLocked } from "../data/job-lifecycle";
...
import { childProgressLabel, type JobDependencyEdge } from "../data/job-tree";
import { UsageRootSummary } from "./usage-root-summary";

type Props = {agents:Agent[];projects?:{id:string;name:string;members?:readonly string[];hostName?:string|null}[];departments?:{id:string;name:string;members?:readonly string[];lead?:string}[];job:Job;jobs:Job[];update:(j:Job)=>void|Promise<boolean>;addJob:(j:Job)=>void|Promise<boolean>;openJob:(id:string)=>void;openAgent?:(id:string)=>void;openDepartment?:(id:string)=>void;openProject?:(id:string)=>void;back:()=>void;notice:(s:string)=>void;openRun:(runId:string)=>void;openUsage?:(recordId:string)=>void;runs?:import("./data").DemoRun[];demoMode?:boolean};
const nextState: Partial<Record<State, string>> = {
 ...
 running:"Исполнитель готовит результат.",review:"Результат ожидает проверки.",waiting_input:"Нужен ответ на вопрос исполнителя.",blocked:"Нужно уточнить вводные перед продолжением.",done:"Результат принят.",canceled:"Задача отменена.",
};
export function JobDetail({agents,projects=[],departments=[],job,jobs,update,addJob,openJob,openAgent,openDepartment,openProject,back,notice,openRun,openUsage,runs=[],demoMode=false}:Props) {
 ...
 const children=jobs.filter(j=>j.parentId===job.id);const parent=jobs.find(j=>j.id===job.parentId);const childProgress=childProgressLabel(children);
 ...
 const confirmPlacement=()=>{
  ...
  if(!next||!canConfirmPlacement(draftBinding,draftDepartment,placementLinks))return;
  change(next,`Проект и отдел: ${next.project} · ${next.department}`);
  setPlacing(false);
 };
 const changeAssignee=(agentId:string)=>{const next=assigneeFields(agentId,agents);change(next,`Исполнитель: ${next.agent}`);};
 ...
 const setStatus=(state:State)=>{if(state!==job.state&&!statusLocked)change({state},`Вы изменили статус: ${stateNames[job.state]} → ${stateNames[state]}`);};
 ...
 const openNativeTab=(hostId:string,path:string,file:TaskFile)=>{
  ...
  if(!navigate.experimental_openFilePreview({target:{kind:'host',hostId,path},location:null}))notice('Не удалось открыть документ в панели BB.');
 };
 const openDocument=async(file:TaskFile)=>{
  const sequence=++openSequence.current;
  if(!demoMode){
   if(!(job.recordId&&isOpaqueRecordId(file.id)&&file.version)){
    notice('Сначала сохраните файл.');
    return;
   }
  ...
  try{
   ...
   if(sequence!==openSequence.current)return;
   if(!target.demo){notice('Предпросмотр не является демо-копией. Реальный файл этим методом не открывается.');return;}
 ...
 const saveFile=async(content?:string):Promise<boolean>=>{
  ...
  const finished=await commitDocumentSave({
   ...
   afterSuccess:async(published)=>{
    ...
    else change({files:[...(job.files||[]).filter(f=>f.id!==saved.id),saved],...(job.state==="done"?{state:"review" as const}:{})},`Вы сохранили ${saved.name}, версию ${saved.version}`);
    setPreview(saved);
 ...
 const initial:TaskActivity[]=demoHistory.length?demoHistory:demoMode?job.comments.map((text,i)=>({id:`initial-${i}`,kind:"comment",author:text.split(":")[0],role:"Участник",text:text.includes(":")?text.slice(text.indexOf(":")+1).trim():text,at:""})):[];
 ...
 const attach=(added:TaskFile[])=>{
  ...
  change({files:[...(job.files||[]),...added]},`Вы прикрепили: ${added.map(f=>f.name).join(", ")}`);
 };
 const createChild=async()=>{
  if(!childTitle.trim()||childPending)return;
  setChildPending(true);
  const created:Job={id:nextJobKey(jobs.map(item=>item.id)),title:childTitle.trim(),parentId:job.id,state:"backlog",project:job.project,department:job.department,agent:job.agent,priority:"Обычный",due:job.due,description:"Опишите ожидаемый результат и критерии приёмки.",comments:[],bindingId:job.bindingId,departmentId:job.departmentId,assignedAgentId:job.assignedAgentId};
 ...
 return <article data-document-visible={documentVisible} className="agency-task-layout mx-auto w-full max-w-7xl px-1 pb-8 sm:px-4">
  <nav aria-label="Путь задачи" className="mb-5 flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground"><Button size="sm" variant="ghost" className="h-auto p-0 font-normal text-muted-foreground hover:text-foreground hover:bg-transparent" onClick={back}>Все задачи</Button><span>/</span>{parent&&<><Button size="sm" variant="ghost" className="h-auto p-0 font-normal text-muted-foreground hover:text-foreground hover:bg-transparent" onClick={()=>openJob(parent.id)}>{parent.id}</Button><span>/</span></>}<span className="font-mono text-foreground">{job.id}</span></nav>
  <div className="agency-task-grid">
   <header className="agency-task-title">
    <div className="agency-task-heading">
     <h1 className="agency-task-heading-title">{job.title}</h1>
     <div className="agency-task-heading-actions">
      {(demoMode||!launchReady)&&<Handoff job={job} agents={agents} files={files} update={update} runtimeAvailable={demoMode}/>}
      <Button size="sm" variant="outline" onClick={openEdit}>Редактировать</Button>
     </div>
    </div>
    <div className="agency-task-heading-meta text-muted-foreground">
     ...
     {job.project&&<><span>·</span><span>{`Проект: ${job.project}`}</span></>}
     {job.department&&<><span>·</span><span>{`Отдел: ${job.department}`}</span></>}
    </div>
   </header>
   ...
   {!question&&
   <section aria-label="Текущее состояние" className={`mb-4 rounded-xl border p-4 ${job.state==="review"?"border-foreground/20 bg-muted/60":"border-border bg-muted/30"}`}>
    <div className="flex items-start gap-3"><span className="mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-full border border-border bg-background">{job.state==="review"?<Icon name="Eye" className="size-4 text-blue-500"/>:job.state==="running"?<Icon name="Play" className="size-4 text-amber-500"/>:job.state==="done"?<Icon name="CircleCheck" className="size-4 text-emerald-500"/>:job.state==="blocked"||job.state==="waiting_input"?<Icon name="AlertCircle" className="size-4 text-orange-500"/>:<Icon name="Circle" className="size-4 text-muted-foreground"/>}</span><div className="min-w-0 flex-1"><span className="text-[11px] font-medium uppercase tracking-wider text-muted-foreground">{job.state==="review"?"Нужно ваше решение":"Текущий этап"}</span><h2 className="mt-0.5 text-sm font-semibold">{job.state==="review"?"Проверьте результат работы":job.state==="done"?"Результат принят":nextState[job.state]}</h2>   <p className="mt-1 text-xs text-muted-foreground">{`Исполнитель: ${job.agent}`}</p>
    {files[0]&&<div className="mt-4 flex flex-wrap gap-2"><FileChip file={files[0]} open={selectFile}/></div>}
    <div className="mt-4 flex flex-wrap items-center gap-2">{job.state==="review"&&<><JobAcceptControls job={job} files={files} api={api} demoMode={Boolean(demoMode)} notice={notice} onAccepted={(text)=>demoMode?change({state:"done"},text):change({},text)}/><Button size="sm" variant="outline" onClick={()=>setReviewing(true)}>Вернуть с замечанием</Button></>}{job.state==="backlog"&&demoMode&&<Button size="sm" onClick={()=>setStatus("queued")}>Передать в работу</Button>}</div>
   ...
   <section className="py-3"><h2 className="mb-3 text-sm font-semibold">Описание и критерии</h2><div className="max-w-prose text-sm text-foreground"><Markdown content={brief.brief}/>{brief.acceptance&&<><h3 className="mb-1.5 mt-4 text-[13px] font-semibold">Критерии приёмки</h3><Markdown content={brief.acceptance}/></>}</div></section>
   <section className="border-t border-border py-4"><div className="mb-3 flex items-center justify-between gap-2"><h2 className="text-sm font-medium">Подзадачи {childProgress&&<span className="ml-1 text-xs font-normal text-muted-foreground">{childProgress}</span>}</h2><Button size="sm" variant="ghost" onClick={()=>setChildForm(true)}>+ Добавить подзадачу</Button></div>{children.length?children.map(child=>{const childTitle=child.title.startsWith(child.id)?child.title.slice(child.id.length).replace(/^[\s:—–-]+/,""):child.title;const cleanAgent=shortAgentName(child.agent);return <button key={child.id} onClick={()=>openJob(child.id)} className="flex min-h-10 w-full items-center gap-2.5 rounded border-b border-border px-1.5 py-2 text-left text-sm hover:bg-muted/40 focus-visible:outline focus-visible:outline-ring"><Status state={child.state} iconOnly/><span className="w-16 shrink-0 font-mono text-xs text-muted-foreground">{child.id}</span><span className="min-w-0 flex-1 truncate font-medium text-foreground">{childTitle}</span><span className="text-xs text-muted-foreground">{cleanAgent?`${cleanAgent} · `:""}{stateNames[child.state]}</span></button>;}):<p className="text-xs text-muted-foreground">Подзадач пока нет. Добавьте отдельный результат, который нужно поручить.</p>}</section>
   <section className="mt-5 border-t border-border pt-5" aria-label="История задачи"><div className="mb-3 flex items-center justify-between gap-3"><h2 className="text-sm font-semibold">История и обсуждение</h2><div className="w-40"><Choice label="Фильтр истории" value={filter} onChange={setFilter} options={[{value:"all",label:"Всё"},{value:"comment",label:"Комментарии"},{value:"event",label:"События"}]}/></div></div>
    {activity.length?<ol className="ml-4 border-l border-border">{activity.map(item=>{if(item.kind==="event")return <li key={item.id} className="relative py-2 pl-6"><span className="absolute -left-1 top-3 size-2 rounded-full border border-border bg-background"/><p className="text-xs leading-snug text-muted-foreground">{item.author&&<span className="mr-2 font-medium text-foreground">{item.author}</span>}{item.text}<span className="ml-2 opacity-70">{item.at&&new Date(item.at).toLocaleTimeString("ru",{hour:"2-digit",minute:"2-digit"})}</span></p></li>;const author=item.author;const agent=author?agents.find(a=>a.name.toLowerCase()===author.toLowerCase()||a.id===author||a.name.toLowerCase().includes(author.toLowerCase())||author.toLowerCase().includes(a.name.toLowerCase())):undefined;const providerId=item.providerId||agent?.selection?.providerId||(item.model?.includes("claude")||item.model?.includes("sonnet")||item.model?.includes("opus")||author?.toLowerCase().includes("fable")||author?.toLowerCase().includes("sonnet")||author?.toLowerCase().includes("opus")?"claude-code":item.model?.includes("gpt")||item.model?.includes("codex")?"codex":undefined);const model=item.model||agent?.selection?.model;const reasoning=item.reasoningEffort||agent?.selection?.reasoningLevel;const role=item.role||agent?.role;const roleLabel=activityRoleLabel(author,role);return <li key={item.id} className="relative py-3 pl-6"><span className="absolute -left-3.5 top-3 flex size-7 items-center justify-center rounded-full border border-border bg-background">{author==="Вы"?<Icon name="UserRound" className="size-3.5 text-muted-foreground"/>:providerId?<AgentMark id={providerId} className="size-4"/>:<Icon name={author==="Система"?"Info":"Bot"} className="size-3.5 text-muted-foreground"/>}</span><div className="mb-1 flex flex-wrap items-baseline gap-x-2 gap-y-1"><span className="text-sm font-medium">{author||"Вы"}</span>{roleLabel&&<span className="text-xs text-muted-foreground">{roleLabel}</span>}{model&&<span className="rounded bg-muted px-1.5 py-0.5 font-mono text-[11px] text-muted-foreground">{model}{reasoning&&reasoning!=="none"?` · reasoning: ${reasoning}`:""}</span>}<span className="text-xs text-muted-foreground">{item.at?<time dateTime={item.at}>{new Date(item.at).toLocaleTimeString("ru",{hour:"2-digit",minute:"2-digit"})}</time>:"Начало примера"}</span></div>{item.text&&<div className="max-w-prose whitespace-pre-wrap break-words text-sm leading-snug">{item.text.replace(/^Вы: /,"")}</div>}<div className="mt-2 flex flex-wrap gap-2">{(item.fileIds||[]).map(id=>files.find(f=>f.id===id)).filter((f):f is TaskFile=>Boolean(f)).map(file=><FileChip key={file.id} file={file} open={selectFile}/>)}</div></li>;})}</ol>:<p className="mb-4 text-xs text-muted-foreground">Здесь пока нет сообщений.</p>}
    <div className="mt-5 rounded-xl border border-border bg-muted/20 p-3"><label htmlFor={`comment-${job.id}`} className="sr-only">Комментарий к задаче</label><textarea id={`comment-${job.id}`} rows={3} value={comment} onChange={e=>setComment(e.target.value)} placeholder="Написать комментарий…" className="w-full resize-y rounded-md bg-transparent p-2 text-sm outline-none focus-visible:ring-1 focus-visible:ring-ring"/><div className="mb-2 flex flex-wrap gap-2">{pendingFiles.map(file=><div key={file.id} className="flex items-center gap-1"><FileChip file={file} open={selectFile}/><Button variant="ghost" size="sm" aria-label={`Убрать ${file.name}`} onClick={()=>setPendingFiles(pendingFiles.filter(f=>f.id!==file.id))}>×</Button></div>)}</div><div className="flex flex-wrap items-center justify-between gap-2"><FilePicker label="Добавить файл" onFiles={added=>setPendingFiles([...pendingFiles,...added])} notice={notice}/><Button size="sm" disabled={!comment.trim()&&!pendingFiles.length} onClick={submitComment}>Отправить комментарий</Button></div></div><p className="mt-2 text-xs text-muted-foreground">{needsInput?NEEDS_INPUT_COMMENT_HINT:"Комментарий пишется в историю задачи. Агент не запускается."}</p>
   </section>


   </div>
   <aside aria-label="Сведения о задаче" className="agency-task-sidebar" data-expanded={detailsOpen}>
    <h2 className="agency-task-sidebar-heading sr-only">Сведения</h2>
    <Button variant="ghost" className="agency-task-sidebar-toggle h-auto min-h-9 w-full justify-between rounded-lg border border-border px-3 text-left" aria-expanded={detailsOpen} aria-controls={`task-details-${job.id}`} onClick={()=>setDetailsOpen(!detailsOpen)}><span>Сведения</span><Icon name={detailsOpen?"ChevronUp":"ChevronDown"} className="size-4"/></Button>
    <div id={`task-details-${job.id}`} className="agency-task-sidebar-content">
     <RailCard title="Команда">
      <RailRow label="Руководитель" value={railTeam.lead?<RailPerson person={railTeam.lead} open={openAgent} short/>:TEAM_UNASSIGNED_ONE} tone={railTeam.lead?"default":"muted"}/>
      <RailRow label="Исполнитель" value={railTeam.assigned?<RailPerson person={railTeam.assigned} open={openAgent} short/>:TEAM_UNASSIGNED_ONE} tone={railTeam.assigned?"default":"muted"}/>
      <RailRow label={railTeam.reviewers.length>1?"Проверяющие":"Проверяющий"} value={railTeam.reviewers.length?<ul>{railTeam.reviewers.map(person=><li key={person.id}><RailPerson person={person} open={openAgent} short/></li>)}</ul>:TEAM_UNASSIGNED} tone={railTeam.reviewers.length?"default":"muted"}/>
     ...
     <RailCard title="Параметры">
      <RailRow label="Проект" value={projectName&&liveBinding&&openProject?<Button size="sm" variant="ghost" className="h-auto px-0" onClick={()=>openProject(liveBinding)}>{projectName}</Button>:projectName||"не задан"} tone={projectName?"default":"muted"}/>
      <RailRow label="Отдел" value={departmentName&&liveDepartment&&openDepartment?<Button size="sm" variant="ghost" className="h-auto px-0" onClick={()=>openDepartment(liveDepartment)}>{departmentName}</Button>:departmentName||"не задан"} tone={departmentName?"default":"muted"}/>
      <RailRow label="Приоритет" value={job.priority}/>
      <RailRow label="Срок" value={job.due||"не задан"} tone={job.due?"default":"muted"}/>
     </RailCard>
     <RailCard title={`Файлы (${files.length})`} label="Файлы" action={<FilePicker onFiles={attach} notice={notice} label="+ Добавить" icon={false}/>}>
      {files.length
       ?<div className="agency-rail-files">{files.map(file=><FileChip key={file.id} file={file} open={selectFile}/>)}</div>
       :<p className="agency-rail-empty">Прикрепите материалы к задаче или сообщению.</p>}
     </RailCard>
     {!demoMode&&<RailCard title="Среда запуска">
      <RailRow label="Хост" value={hostName} tone={hostName===HOST_UNKNOWN?"muted":"default"}/>
      <RailRow label="Изоляция" value={isolation.text} tone={isolation.tone}/>
      <RailRow label="Попыток" value={attemptsLabel(environment)} mono/>
     </RailCard>}
     {demoMode&&<RailCard title="Запуски примера" label="Запуски примера">
      {jobRuns.length?jobRuns.map(run=><div key={run.id} className="flex flex-col items-start gap-2 py-1"><div className="flex items-center gap-2 text-xs"><span className="flex size-7 shrink-0 items-center justify-center rounded-full border border-border bg-muted"><AgentMark id={run.providerId} className="size-4"/></span><span>{run.agent} · {run.title}</span></div><Button size="sm" variant="outline" onClick={()=>openRun(run.id)}>Открыть запуск</Button></div>):<p className="agency-rail-empty">Запусков в примере нет.</p>}
     </RailCard>}
     {!demoMode&&job.recordId&&openUsage&&<RailCard title="Расход токенов" label="Расход">
      <UsageRootSummary api={api} rootJobId={job.recordId} revision={job.revision} openUsage={()=>openUsage(job.recordId!)}/>
     </RailCard>}
     <RailCard title="Иерархия и связи">
      <details className="group text-xs" open={treeOpen} onToggle={e=>setTreeOpen(e.currentTarget.open)}>
       <summary className="flex cursor-pointer list-none items-center justify-between py-0.5 text-muted-foreground hover:text-foreground [&::-webkit-details-marker]:hidden">
        <span>{treeOpen?"Свернуть связи":"Показать связи"}</span>
  ...
  <JobEditDialog open={editing} title={editDraft.title} description={editDraft.description} pending={editPending} onOpenChange={setEditing} onTitle={value=>patchEditDraft({title:value})} onDescription={value=>patchEditDraft({description:value})} onSave={saveEdit}>
   <div className="grid gap-3 sm:grid-cols-2">
    <Field label="Статус"><Choice label="Статус задачи" value={editDraft.state} onChange={value=>patchEditDraft({state:value as State})} options={states.map(value=>({value,label:stateNames[value]}))} disabled={statusLocked}/></Field>
   ...
   {statusLocked&&<p className="text-xs text-muted-foreground">Пока открыт запрос ввода, статус задачи не меняется отсюда.</p>}
   <Field label="Проект и отдел" hint="Пара сохраняется сразу, отдельным подтверждением.">
    <div className="flex flex-wrap items-center justify-between gap-2">
     <span className="text-sm">{`${projectName||"проект не задан"} · ${departmentName||"отдел не задан"}`}</span>
     {demoMode
      ?<div className="flex flex-wrap gap-2"><Choice label="Проект задачи" value={job.project} onChange={project=>change({project},`Проект: ${project}`)} options={projects.map(p=>p.name)}/><Choice label="Отдел задачи" value={job.department} onChange={department=>change({department},`Отдел: ${department}`)} options={departments.map(d=>d.name)}/></div>
      :<Button size="sm" variant="outline" onClick={openPlacement}>Изменить</Button>}
    </div>
   </Field>
  ...
  <Dialog open={placing} onOpenChange={setPlacing}><DialogContent><DialogHeader><DialogTitle>Проект и отдел</DialogTitle><DialogDescription>Сохраняется только подтверждённая пара из связей проекта. Смена проекта сбрасывает отдел, которого нет в связях.</DialogDescription></DialogHeader><Choice label="Проект задачи" value={draftBinding} onChange={changeDraftBinding} options={projects.map(item=>({value:item.id,label:item.name}))}/>{draftDepartments.length?<Choice label="Отдел задачи" value={draftDepartment} onChange={setDraftDepartment} options={draftDepartments.map(item=>({value:item.id,label:item.name}))}/>:<p className="text-xs text-muted-foreground">{draftBinding?JOB_CREATE_NO_DEPARTMENTS:"Сначала выберите проект."}</p>}<DialogFooter><Button variant="outline" onClick={()=>setPlacing(false)}>Отмена</Button><Button disabled={!canConfirmPlacement(draftBinding,draftDepartment,placementLinks)} onClick={confirmPlacement}>Сохранить пару</Button></DialogFooter></DialogContent></Dialog>
  <Dialog open={childForm} onOpenChange={open=>{if(!childPending)setChildForm(open);}}><DialogContent><DialogHeader><DialogTitle>Новая подзадача</DialogTitle><DialogDescription>Будет связана с {job.id} и унаследует проект и отдел родителя. Ключ назначит каталог после сохранения.</DialogDescription></DialogHeader><TextField label="Название подзадачи" value={childTitle} onChange={setChildTitle}/><p className="text-xs text-muted-foreground" aria-live="polite">{childPending?"Сохраняем подзадачу…":"Поле останется, пока сервер не подтвердит запись."}</p><DialogFooter><Button variant="outline" disabled={childPending} onClick={()=>setChildForm(false)}>Отмена</Button><Button disabled={!childTitle.trim()||childPending} onClick={()=>void createChild()}>{childPending?"Сохраняем…":"Создать подзадачу"}</Button></DialogFooter></DialogContent></Dialog>
  <Dialog open={Boolean(pendingPreview)} onOpenChange={open=>{if(!open&&!fileSaving)setPendingPreview(null);}}><DialogContent><DialogHeader><DialogTitle>В файле есть несохранённые изменения</DialogTitle><DialogDescription>Сохраните правки или отбросьте их перед переходом.</DialogDescription></DialogHeader>{fileSaveError&&<p className="text-xs text-muted-foreground">{fileSaveError}</p>}<DialogFooter><Button variant="outline" disabled={fileSaving} onClick={()=>setPendingPreview(null)}>Продолжить редактирование</Button><Button variant="outline" disabled={fileSaving} onClick={discardFile}>Отбросить правки</Button><Button disabled={fileSaving} onClick={()=>{void (async()=>{const next=pendingPreview?.file||null;const ok=await saveFile();if(!canLeaveAfterSave(ok))return;setPendingPreview(null);setPreview(next);if(next)void openDocument(next);})();}}>{fileSaving?"Сохраняем…":"Сохранить и перейти"}</Button></DialogFooter></DialogContent></Dialog>
  <Dialog open={reviewing} onOpenChange={setReviewing}><DialogContent><DialogHeader><DialogTitle>Вернуть результат на доработку</DialogTitle><DialogDescription>Укажите, что требуется изменить в результате.</DialogDescription></DialogHeader><TextField label="Замечания" value={reason} onChange={setReason} multiline/><DialogFooter><Button variant="outline" onClick={()=>setReviewing(false)}>Отмена</Button><Button disabled={!reason.trim()} onClick={()=>{change({state:"running"},`Вы вернули результат: ${reason.trim()}`);setReason("");setReviewing(false);}}>Вернуть на доработку</Button></DialogFooter></DialogContent></Dialog>
 </article>;
}
